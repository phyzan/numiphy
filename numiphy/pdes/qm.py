...
class Schroedinger1D(ivps.HomLinearIBVP):
    ...
    def __init__(self, V: Expr, grid: grids.Grid1D, hbar=1., m=1.):
        x = Symbol('x', 0)
        Dx = Diff(x)
    ...
    def get_ScalarField(self, t, acc=1, fd='central', **ode_args):
        return super().get_ScalarField('Ψ', t, acc, fd, **ode_args)
