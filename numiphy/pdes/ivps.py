...
class IVP(ABC):
    ...
    def solve(self, t, dt, method='RK4', err=0., max_frames=-1, display=True, thres=0.0, acc=1, fd='central'):
        '''
        remember to include non-autonomous IVP's
        '''
        ics = (0, self.bcs.discretize(self.f0, 0))
        kwargs = dict(t=t, dt=dt, method=method, err=err, max_frames=max_frames, display=display, thres=thres)
    ...
    def get_ScalarField(self, name: str, t, dt, method='RK4', err=0., max_frames=-1, display=True, thres=0.0, acc=1, fd='central'):
        '''
        ...
        t, f = self.solve(t=t, dt=dt, method=method, err=err, max_frames=max_frames, display=display, thres=thres, acc=acc, fd=fd)
...
class HomLinearIBVP(HomLinearIVP):
    ...
    def solve(self, t, dt, method='RK4', err=0., max_frames=-1, display=True, thres=0.0, acc=1, fd='central'):
        if method == 'propagator':
            ...
            return super().solve(t, dt, method, err, max_frames, display, thres, acc, fd)
...
class Wave(IVPsystem2D, HomLinearIVP):
    ...
    def solve(self, t, dt, method='RK4', err=0., max_frames=-1, display=True, thres=0.0, acc=1, fd='central'):
        x, (f, df) = IVPsystem2D.solve(self, t, dt, method, err, max_frames, display, thres, acc, fd)
        return x, f
